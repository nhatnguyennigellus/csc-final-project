...
public class AccountServiceTest {
	...
	public void testSearchAccount() {
		String idCard = "024643151";
		String accNumber = "";
		List<SavingAccount> accounts = bankingService.searchSavingAccounts(
				idCard, accNumber);
		assertTrue(!accounts.isEmpty());
		assertTrue(accounts.get(0).getCustomer().getIdCardNumber()
				.equals(idCard));
	}
	...
	public void testGenerateAccountNumber() {
		...
		assertTrue("Account length is not 12 -> " + genAccountNumber.length(),
				genAccountNumber.length() == 12);
		assertTrue("Wrong year -> " + genAccountNumber.substring(0, 2),
				genAccountNumber.substring(0, 2).equals("15"));
